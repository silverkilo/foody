        ...
        socket.on('getPotentialMatches', async () => {
            try {
                ...
                const [matchers] = await db.query(`
                    ...
                    LIMIT 5;
                ;
                `, { replacements: [user.preferences, user.id, exclusions[userId], user.location.coordinates[0], user.location.coordinates[1]] })
                if (matchers.length < 5) {
                    ...
                    const [moreMatchers] = await db.query(`
                        ...
                        ORDER BY "user"."location" <-> 'SRID=26918;POINT(? ?)'::geometry
                        LIMIT ?;
                `, { replacements: [user.preferences, exclusions[userId].concat(matchers.map(({ id }) => id)), user.location.coordinates[0], user.location.coordinates[1], 5 - matchers.length] })
            ...
            try {
                ...
                if (user.hasMatched) {
                    ...
                    if (matcheeInfo.socketId) {
                        socket.emit({ matched: true, matcheeInfo })
                        return socket.to(matcheeInfo.socketId).emit({ matched: true, matcherInfo: user })
                    }
                }
                ...
                if (value) {
                    ...
                    if (matched) {
                        ...
                        if (matcherInfo.socketId && matcheeInfo.socketId) {
                            ...
                            return socket.to(matchee.socketId).emit('didMatch', { matched, matcherInfo })
                        } else {
